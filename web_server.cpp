...
WebServer::WebServer() {
    ...
    m_users = new http::HttpConn[MAX_FD + 1];
    m_users_timer = new client_data[MAX_FD + 1];
}
...
bool WebServer::InitThreadPool() {
    m_threadpool = std::make_shared<thread_pool<http::HttpConn>>(m_actor_model, m_conn_pool, m_thread_num);
...
bool WebServer::Init(WebServerConfig&& config) {
    ...
    m_trig_mode = config.m_trig_mode;
    ...
    if (m_actor_model == ProactorMode) {
        std::cout << "Web Server Mode : ProactorMode" << std::endl;
    } else {
        std::cout << "Web Server Mode : Reactor" << std::endl;
    }

    
    if (m_log_status == LogEnable) {
        std::cout << "Log System Open!" << std::endl;
        std::cout << "Log Mode : " << (m_log_mode == LogAsyncMode ? "AsyncMode" : "SyncMode") << std::endl;
    } else {
        std::cout << "Log System Close!" << std::endl;
    }
    
    if (m_listenfd_trig_mode == LevelTriggerMode) {
        std::cout << "Listen Fd : LevelTriggerMode" << std::endl;
    } else {
        std::cout << "Listen Fd : EdgeTriggerMode" << std::endl;
    }
    if (m_conn_trig_mode == LevelTriggerMode) {
        std::cout << "Connection Fd : LevelTriggerMode" << std::endl;
    } else {
        std::cout << "Connection Fd : EdgeTriggerMode" << std::endl;
    }
...
void WebServer::SetTimer(int connfd, struct sockaddr_in client_address) {
    ...
    time_t cur = time(NULL); //system time, from 1970.0.0->now seconds
...
void WebServer::delete_timer(util_timer* timer, int sockfd) {
    ...
    LOG_INFO("close socketfd %d", m_users_timer[sockfd].sockfd);
}
...
bool WebServer::deal_client_data() {
    ...
    // LT
    if (LevelTriggerMode == m_listenfd_trig_mode) {
        int connfd = accept(m_listenfd, (struct sockaddr *)(&client_addr), &client_len);
        if (connfd < 0) {
            LOG_ERROR("%s:errno is:%d", "accept error", errno);
            return false;
        }
        if (http::HttpConn::m_user_count >= MAX_FD || (size_t)connfd >= MAX_FD) {
    ...
    if (ReactorMode == m_actor_model) { //reactor
        ...
        while (true) {
            if (ConnIsDeal == m_users[sockfd].m_isdeal) {
                ...
                m_users[sockfd].m_isdeal = ConnUnDeal;  //循环置为未处理
                break;
            }
        }
    } else { //proactor async 
...
void WebServer::Run() {
    ...
    while (stop_server == false) {
        int num = epoll_wait(m_epollfd, m_events, MAX_EVENT_NUMBER, -1); //非阻塞
        ...
        for (int i = 0; i < num; ++ i) {
            ...
            if (sockfd == m_listenfd) {
                ...
                m_request_num = (m_request_num + 1) % 0xffffffff; // 统计请求数
                LOG_INFO("request num = %d", m_request_num);
                ...
                if (flag == false) {
                    LOG_ERROR("%s", "deal with timer signal failure");
                }
