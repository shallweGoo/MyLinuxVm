...
void HttpConn::Init(int sockfd, const sockaddr_in &addr, const char* root_path, int trig_mode, int log_status, 
                    string user, string passwd, string sqlname, string tablename) {
    m_sockfd = sockfd;
    m_address = addr;
    addfd(m_epollfd, m_sockfd, true, trig_mode);
    ++m_user_count;
...
HTTP_CODE HttpConn::parse_headers(const char* line) {
    ...
    if (strncasecmp(line, CONNECTION.c_str(), CONNECTION.size()) == 0 && strcasecmp(line + CONNECTION.size(), "keep-alive") == 0) {
        ...
        // LOG_INFO("unsupport header: %s", line);
    }
    return NO_REQUEST;
}
...
bool HttpConn::add_cookie(const char* key, const char* value) {
    return add_rsp("Set-Cookie:%s=%s\r\n", key, value);
}
...
void HttpConn::CloseConn(bool real_close) {
    if (real_close && (m_sockfd != -1)) {
        LOG_INFO("%s", "close http connection");
        --m_user_count;
...
void HttpConn::process() {
    ...
    LOG_INFO("%s", "process read success");
    bool write_ret = process_write(read_ret);
    if (write_ret == false) {
        LOG_ERROR("%s", "process write fail");
        CloseConn();
    }
    modfd(m_epollfd, m_sockfd, EPOLLOUT, m_trig_mode); // epoll can write
    LOG_INFO("%s", "process write success");
}
